...
八叉树，EPA，包括最后的冲量积分都会受到访问检测顺序的影响，甚至是Player驱动的调用
...
### 回滚备份，CollisionManager因为依赖上一帧的碰撞结果，因此CollisionManager也必须参与回滚当中

#### 回滚问题1：回滚的必要性

回滚是必要的，因为Collision我们有一些优化，比如Prelist，Dirty这一些，是依赖上一帧状态的，如果上一帧状态是错误的，那么结果就会出问题。

同时，回滚也并不是在两个客户端都会发生的，如果一个回滚了一个没回滚，也就是一个状态是对的，一个状态是错的，那就会导致错乱

#### 回滚问题2：RollManager注册先后问题

在回滚时，必须先重置“环境（CollisionManager）”，再恢复“物体（Entity）”

我们有一个优化是，我们碰撞检测并不是检测所有的物体，而是检测上一帧触发物理的和上一帧移动的，以及这一帧为Dirty的（也就是这一帧主动运动的）

如果回滚先回滚了Entity。Entiy的碰撞体会为Dirty，但是回滚Collision 的时候会先清空Dirty列表。这会导致在两个客户端上，回滚的那个客户端的Dirty是空的，但是没回滚的那个是有Dirty的。

> 如果 `Entity` 先恢复，它会调用 `SetDirtyCollider` 试图告诉物理引擎“我醒了”。紧接着 `CollisionManager` 恢复，它在 `Restore` 里直接 `Clear` 了所有脏标记。结果就是：物体虽然位置对了，但物理引擎认为它是睡着的，导致它在挤压时不产生反作用力，两端客户端表现不一致。

### 一致性ID

### 确定性调用顺序

比如我有一个ID生成器，但是ID生成器的调用放在了Awake里，Unity不保证Awake的调用顺序

### 

### GJK奇异点造成的问题

### 双重位移积分（Double Integration）

就是我Logic层也在修改位置，但是实际上应该完全交给Collision层处理

这是导致物理不稳定的根源，特别是在挤压场景下。

- **现象**：您的代码中，玩家的移动被执行了两次：
  1. **逻辑层** (`PlayerEntity`): `Position += Velocity * dt` （手动强行位移）。
  2. **物理层** (`CollisionManager`): `proxy.Integrate(dt)` （物理引擎位移）。
- **后果**：
  - 玩家移动速度比预期快。
  - 更严重的是，当物理引擎检测到碰撞（比如撞到箱子或墙）并将物体弹开或刹停后，**逻辑层完全不知道**。
  - 下一帧，逻辑层依然用满速强行把玩家“按”进墙里。物理引擎再费劲把它“弹”出来。这种“按进去-弹出来”的高频震荡在网络同步中极度脆弱，微小的计算误差就会导致两端结果完全不同。
...
## 改进：**离散检测的子步进 (Discrete Sub-stepping)** / **求解器迭代 (Solver Iterations)**  + 八叉树的重建
