...
InteractSystem的DoUpdate查询所有的实体的位置，对于可以交互的实体，加让一个CanInteractCompoent

BurnSystem的DoUpdate，查询遍历所有同时有`InteractionTargetComponent` **和** `BurnComponent` **和** `HealthComponent`的实体

然后根据实体血量是否是满的，向HUDSystem注册一个维修的回调（最好是个闭包）

然后HUDSytem就处理按键按下

> 折中的写法：

上层有一个InteractSystem，这个System的逻辑是根据各个实体的位置检查是否能交互

所有下层的System都要实现一个CanInteract方法，这个方法根据格子的不同来实现

InteractSystem检测到某个实体可以交互，就遍历它上面的Component，如果发现存在BurnCompoent，就调用BurnSystem的CanInteract方法，如果为True，就调用BurnSystem的Interact方法

BurnSystem里，根据当前实体的血量是否不满来返回CanInteract方法。

BurnSystem的Inetract方法负责把点燃的按钮（比如说是B）注册到HUDSystem的按钮列表里

当按下按钮的时候，回调会调用BurnSystem里的具体的维修方法

> 折中设计的好处，BurnSystem.UnRegister，比如Lua中，回调是一个闭包，这样的话，在离开区域的时候，就可以让InteractSystem调用BurnSystem的反注册方法